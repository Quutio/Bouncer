...
internal class DistributedServer(
	override val info: IDistributedServerInfo,
	state: IDistributedServerState) : RegisteredBouncerEntity(), IDistributedServer
{
	...
	private fun sendUpdate(update: ServerUpdate)
	{
		...
		sessionData.session.sendUpdate(serverUpdateRequest()
		{
	...
	internal fun registerUniverse(options: IDistributedUniverseOptions, supervisor: IDistributedUniverseSupervisorInstanceOptions): IDistributedUniverseContainer
	{
		...
		return DistributedUniverseContainer(universe, this::unregisterUniverse)
	}
