...
internal sealed class QueueManager
{
	...
	private readonly ConcurrentDictionary<string, ConcurrentBag<RegisteredQueue>> gameQueues = [];
	...
	internal void Join(RegisteredQueue queue, string gamemode)
	{
		ConcurrentBag<RegisteredQueue> queues = this.gameQueues.GetOrAdd(gamemode, _ => []);
