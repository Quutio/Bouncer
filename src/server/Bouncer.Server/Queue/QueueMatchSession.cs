...
internal sealed class QueueMatchSession(ImmutableArray<RegisteredQueue> queues)
{
	...
	{
		List<Task<UniverseQueueRequest.Types.ConfirmPrepapre>> prepares = [];
		foreach ((QueueSession session, HashSet<RegisteredQueue> sessionQueues) in this.queuesBySession)
		{
			prepares.Add(session.WriteAsync(new UniverseQueueResponse.Types.Prepapre
			{
		...
		foreach ((QueueSession session, HashSet<RegisteredQueue> sessionQueues) in this.queuesBySession)
		{
			_ = session.WriteAndForgetAsync(new UniverseQueueResponse.Types.Confirmation
			{
				QueueIds = { sessionQueues.Select(i => i.Id) }
			});
		}
